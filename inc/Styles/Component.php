...
use function WP_Rig\WP_Rig\sbh_center;
...
class Component implements Component_Interface, Templating_Component_Interface
{
	 ...
	 * $data must be an array with keys 'file' (file path relative to 'css' directory), and optionally 'global'
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `sbh_center()`.
	 *
	...
	public function action_enqueue_styles()
	{
		...
		if (!empty($google_fonts_url)) {
			wp_enqueue_style('sbh_center-fonts', $google_fonts_url, [], null); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ($css_files as $handle => $data) {
			...
			$version = sbh_center()->get_asset_version($css_dir . $data['file']);
	...
	{
		if ('preconnect' === $relation_type && wp_style_is('sbh_center-fonts', 'queue')) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function ($handle) use ($css_files) {
				...
				if (!$is_valid) {
					...
					_doing_it_wrong(__CLASS__ . '::print_styles()', esc_html(sprintf(__('Invalid theme stylesheet handle: %s', 'sbh_center'), $handle)), 'SBH Center 2.0.0');
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'sbh_center_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	{
		$preloading_styles_enabled = !sbh_center()->is_amp();
		 ...
		 */
		return apply_filters('sbh_center_preloading_styles_enabled', $preloading_styles_enabled);
	}
	...
	protected function get_css_files(): array
	{
		 ...
		 */
		$css_files = apply_filters('sbh_center_css_files', $css_files);
	...
	protected function get_google_fonts(): array
	{
		 ...
		 */
		$this->google_fonts = (array) apply_filters('sbh_center_google_fonts', $google_fonts);
